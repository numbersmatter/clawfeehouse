...
async function seed() {
  ...
  try {
    ...
    const existingUser = await db
      .select()
      .from(users)
      .where(eq(users.email, 'llawsonwork@gmail.com'))
      .get()

    if (existingUser) {
      console.log('✓ Admin user already exists')
      console.log(`  Email: llawsonwork@gmail.com`)
      console.log(`  Role: ${existingUser.role}`)
      return
    }

    // Hash password using SHA-256 (same as SonicJS auth system)
    const data = 'Need2besonic.' + 'salt-change-in-production'
